import { teacherContext, classMock, getClassMock, subjectLabels, getOfferedSubjectCodesForClass } from "../../shared/scripts/data/mock-data.js";
...
function renderSubjects() {
  ...
  subjectsGrid.innerHTML = subjects
    .map((subject) => {
      ...
      return `
        <button type="button" class="subject-card" data-subject="${escapeHTML(subject.id)}">
          <span class="subject-doodles" aria-hidden="true">
            ${subjectDoodles(subject.id).map((glyph) => `<i>${escapeHTML(glyph)}</i>`).join("")}
          </span>
          <img class="subject-icon" src="${escapeHTML(subject.icon)}" alt="" aria-hidden="true" />
...
// faint background glyphs per subject just to spice the subject cards(visuals)
const SUBJECT_DOODLES = {
  MAT: ["🔢︎", "2⁷", "𝝅", "√", "⏲", "⃤", "𓍝", "⊿", "i³", "✗", "⛀⛁", "∑", "±" ],
  ENG: ["✎", "❝ ❞", "Aa","🗯", "🗣", "❞", "≣", "𓂃✍︎", "ִ🕮", "🗪", "✉︎", "⁉"],
  KIS: ["𓂃🖊", "❝ ❞", "Ss", "⁉", "🗯", "🗣", "ᶻ𐰁", "📖︎", "‼", "≣", "✎", "✉︎", "🗪"],
  SCI: ["🐛︎", "🍂︎", "☢", "⚙", "𓌉◯𓇋", "Co²", "🕸", "🕷", "𓆈","📱︎", "🖨︎", "💻︎", "🌐︎", "🖥︎"],
  SS: ["ᨒ", "🗺", "⛱", "✈", "🏝", "🌤", "🌡", "🏜", "🏞", "👥︎", "✊︎", "🇰🇪", "🌳︎", "🎭︎", "☪︎✝︎", "🏛︎"],
  INT: ["⚗︎", "⋆⌬", "♻", "⚡︎", "☤", "⚧", "⚠︎",  "🔬︎", "🔭︎", "👁︎", "⚛", "🌡︎"],
  COMP: ["🖧", "</>", "💻︎", "💾︎", "🖥︎", "🖳", "모", "⁴⁰⁴", "📡︎", "🗁", "⊞", "🌐︎", "{ }", "🛡︎", "🖨︎", "⛁"],
  CRE: ["†⛪︎†", "♕", "✞", "𓉸", "🕯", "⛧", "🕊", "𓆩†𓆪", "🎚", "🍞︎", "👼︎", "🔔︎"],
  IRE: ["☪︎📿︎", "☪︎", "🕋︎", "۞", "📜︎", "☾⋆", "-`♡´-", "الله", "🕌︎", "🛐︎", "🌙︎", "☝︎", "♔"],
  ENV: ["♲", "🌍︎", "🌱︎", "🌀︎", "💧︎", "🛢", "🌨", "🛏", "☘", "🌲︎", "🌪︎", "🚲︎", "🍽︎", "☠︎", "🐄︎",  "⛰︎"],
  PTECH: ["🛠︎", "⛓", "🔌︎", "🏗", "⛟", "🏠︎", "⛏", "⚙︎", "⌁", "⚠︎", "📐︎", "🗜︎", "⚓︎", "💡︎", "📏︎", "💼︎", "⛑︎", "✂︎", "📣︎"],
  AGRI: ["𓃽𓃽𓀚", "⸙", "𓆝 𓆟 𓆞", "🌱︎", "𓄀", "𓇢𓆸", "𓃖", "𓃟", "🐪︎", "🌾︎", "🌿︎", "☁︎", "🌳︎", "🐞︎", "◉", "🥛︎", "🐓︎"],
  ART: ["⚽︎", "♫", "𓂃🖌", "🎼︎", "🎤︎︎", "⚾︎", "ᯓ♪", "🏃︎", "🏟", "-ˋˏ✄┈┈┈┈", "⛸", "🎨︎", "🎭︎", "⛹︎", "⚕︎"]
};
...
function renderStudentProfilePage(studentId) {
  ...
  // only the subjects this student's grade band actually offers.
  const offeredCodes = getOfferedSubjectCodesForClass(profile.classId);
  if (chartEl) {
    const labels = offeredCodes;
    if (!hasAnyScore(labels, profile.scores)) {
      chartEl.innerHTML = "";
    } else {
      chartEl.innerHTML = buildPerformanceChartSVG({
        labels,
        scores: profile.scores,
        classAverage: profile.classAverage,
        ariaLabel: `${profile.name}'s scores and class average by subject`,
      });
    }
  }
  const subjectKeyEl = $("#subjectKey");
  if (subjectKeyEl) {
    const filteredLabels = Object.fromEntries(
      Object.entries(subjectLabels).filter(([code]) => offeredCodes.includes(code))
    );
    subjectKeyEl.innerHTML = buildSubjectKeyHTML(filteredLabels);
  }
...
function handleRoute() {
  ...
  if (view === "dashboard") {
    ...
    transitionToView("dashboard", null);
    return;
  }
...
function showTeacherLogin() {
  teacherLoginGate.showLogin();
}

function bindTeacherLogin() {
  teacherLoginGate.bind();
}
