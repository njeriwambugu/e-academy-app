...
export function createAssignmentsFeature(deps) {
  ...
  function getLearnerRows(row) {
    ...
    return students.map((student) => {
      ...
      const seconds = override ? override.seconds : score != null ? 72 + (seed % 190) : null;
      const mapped = override
        ? CATEGORY_TO_LEARNER_STATUS.done
        : (CATEGORY_TO_LEARNER_STATUS[category] || CATEGORY_TO_LEARNER_STATUS.pending);
